...
function TextField({ label, value, onChange, multiline, type = 'text' }) {
  return (
    <div className="mb-8">
      <label style={{ fontFamily: 'Cormorant Garamond, Georgia, serif', fontSize: '0.95rem', color: COLORS.ink, display: 'block', marginBottom: '0.5rem', lineHeight: 1.5 }}>
        {label}
      </label>
      {multiline ? (
        <textarea
...
function SelectField({ label, options, value, onChange }) {
  return (
    <div className="mb-10">
      <label style={{ fontFamily: 'Cormorant Garamond, Georgia, serif', fontSize: '0.95rem', color: COLORS.ink, display: 'block', marginBottom: '1rem', lineHeight: 1.5 }}>
        {label}
      </label>
...
function FoundationScreen({ data, update, onNext, onBack }) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-12">
      ...
      {Object.entries(FOUNDATION_QUESTIONS).map(([key, q]) => {
        if (q.type === 'longtext') {
          return <TextField key={key} label={q.label} value={f[key]} multiline onChange={v => update(`foundation.${key}`, v)} />;
        }
        if (q.type === 'select') {
          return <SelectField key={key} label={q.label} options={q.options} value={f[key]} onChange={v => update(`foundation.${key}`, v)} />;
        }
        if (q.type === 'multiselect') {
          return <MultiSelectField key={key} label={q.label} options={q.options} value={f[key]} onChange={v => update(`foundation.${key}`, v)} />;
        }
        return null;
      })}
...
function ReportScreen({ data, onRestart }) {
  ...
  const downloadReport = async () => {
    ...
    try {
      ...
      const docx = await import('https://esm.sh/docx@9.6.1');
